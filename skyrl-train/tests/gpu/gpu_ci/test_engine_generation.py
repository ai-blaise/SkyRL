...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("sglang", 2, 1, 1, marks=pytest.mark.sglang),  # TP=2 for SGLang
    ],
    ids=["vllm_tp2", "vllm_dp2", "vllm_tp2_pp2", "sglang", "sglang_tp2"],
)
def test_inference_engines_generation(ray_init_fixture, backend: str, tp_size: int, pp_size: int, dp_size: int):
    ...
    try:
        # skip_tokenizer_init=True for /generate endpoint (token-in-token-out)
        llm_client, remote_server_process = init_remote_inference_servers(
            tp_size, backend, tokenizer, cfg, MODEL, skip_tokenizer_init=True
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("sglang", 2, 1, marks=pytest.mark.sglang),  # TP=2 for SGLang
    ],
    ids=["vllm_dp2", "sglang", "sglang_tp2"],
)
